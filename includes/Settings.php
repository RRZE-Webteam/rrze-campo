...
use function RRZE\Campo\Config\getFields;
use function RRZE\Campo\Config\getMenuSettings;
use function RRZE\Campo\Config\getOptionName;
use function RRZE\Campo\Config\getSections;
...
class Settings
{
    ...
    public function __construct($pluginFile)
    {
        ...
        $updated = get_option('campo-updated');
        if (!empty($oldOptions) && empty($updated)) {
            foreach ($oldOptions as $k => $v) {
                $oldOptions['basic_' . $k] = $v;
            }
            if (empty($oldOptions['basic_campo_url'])) {
                $oldOptions['basic_campo_url'] = 'https://campo.uni-erlangen.de';
            }
            update_option('rrze-campo', $oldOptions);
            update_option('campo-updated', 1);
        }
    }
    ...
    public function onLoaded()
    {
        ...
        // Save options if they haven't been saved at least once because we need them for ICS (see https://github.com/RRZE-Webteam/rrze-campo/issues/180)
        $storedOptions = get_option('rrze-campo');
        if (empty($storedOptions)) {
            update_option('rrze-campo', $this->options);
        }
